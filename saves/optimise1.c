...
void calculateT(int tSize, mpz_t* T, mpz_t x, mpz_t N, mpz_t omega, mpz_t u2, mpz_t temp, mpz_t b, mpz_t r, unsigned long long t){
    int i = 0;
    int k = 0;
    mpz_t xSquared;
    mpz_inits(T[0], xSquared, NULL);
    mpz_set(T[0], x);
    ZnModMul(xSquared, x, x, N, omega, u2, temp, b, r, t); 
    //mpz_mul(xSquared, x, x);
    //mpz_mod(xSquared, xSquared, N);  
    for(i = 1; i < tSize; i++){
      mpz_init(T[i]);
      ZnModMul(T[i], T[i-1], xSquared, N, omega, u2, temp, b, r, t);
...
void newPow(mpz_t t, mpz_t x, mpz_t y, mpz_t N){
 ...
 // mpz_mod(x, x, N);
  ZnModMul(xBar, x, pSqared, N, omega, u2, temp, b, r, t2);
  ZnModMul(t, one, pSqared , N, omega, u2, temp, b, r, t2);
  calculateT(tSize, T, xBar, N, omega, u2, temp, b, r, t2);
  ...
  int e, c, g;
   while(i >= 0){
      ...
      for(g = 0; g < i-l+1; g++){
         ZnModMul(t, t, t, N, omega, u2, temp, b, r, t2);
      }

        if(u != 0){
            size_t q = (u-1)/2;

            ZnModMul(t, t, T[q], N, omega, u2, temp, b, r, t2);
        }
      i = l-1;
    }
  ...
  ZnModMul(t, t, one, N, omega, u2, temp, b, r, t);
   mpz_clear(xBar);
   mpz_clear(omega);
   mpz_clear(pSqared);
   mpz_clear(one);
...
void ZnModMul(mpz_t result, mpz_t x, mpz_t y, mpz_t N, mpz_t omega, mpz_t u, mpz_t temp, mpz_t b, mpz_t r, unsigned long long t){
  int i;
