...
void calculateT(int tSize, mpz_t* T, mpz_t x, mpz_t N){
    ...
    int k = 0;
    mpz_t doubleX;
    mpz_inits(T[0], doubleX, NULL);
    mpz_set(T[0], x);
    mpz_mul(doubleX, x, x);
    mpz_mod(doubleX, doubleX, N);  
    for(i = 1; i < tSize; i++){
      mpz_init(T[i]);
      mpz_mul(T[i], T[i-1], doubleX);
      mpz_mod(T[i], T[i], N);  
    }
}

void newPow(mpz_t result, mpz_t x, mpz_t y, mpz_t N, int k){
  double m = pow(2,k);
  ...
  while(i >= 0){
      if(mpz_tstbit(y, i) == 0){
        ...
        u = 0;   //NOT SURE IF SHOULD BE 0 or 1!!!!!!
        e = 1;
        for(c = l; c <= i; c++){
         // gmp_printf("y = %Zx\n", y);
          if(mpz_tstbit(y, c) == 1){
